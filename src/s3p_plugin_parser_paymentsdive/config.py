...
config = PluginConfig(
    plugin=CoreConfig(
        reference='paymentsdive',         # уникальное имя источника
        type=SOURCE,                            # Тип источника (SOURCE, ML, PIPELINE)
        files=['payments_dive.py', ],        # Список файлов, которые будут использоваться в плагине (эти файлы будут сохраняться в платформе)
    ...
    payload=payload.PayloadConfig(
        file='payments_dive.py',                 # python файл плагина (точка входа). Этот файл должен быть указан в `plugin.files[*]`
        classname='PaymentsDive',               # имя python класса в указанном файле
